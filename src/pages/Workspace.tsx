...
import {
  ...
  ChevronRight, Check, Plus, UserCircle, Wallet
} from "lucide-react";
...
export default function Workspace() {
  ...
  // welcome toast for personal mode - always show when no enterprise
  ...
  const loadEnterprises = async () => {
    ...
    // Personal mode toast always shows (frontend demo)

    if (list.length === 0) {
      // No enterprises: enter personal mode directly
      setEnterprise(null);
      setRole("member");
      setCurrentOrg(null);
      setLoading(false);
    ...
    <div className="relative" ref={userMenuRef}>
      ...
      >
        ...
        <div className="hidden sm:flex flex-col items-start">
          ...
          <span className="text-[10px] leading-tight px-1.5 py-0.5 rounded bg-primary/10 text-primary border border-primary/20 font-medium">
  ...
  // ── Render personal workspace (no enterprise) ──
  if (!enterprise) {
    return (
    <SidebarProvider>
      <div className="min-h-screen flex w-full">
        <WorkspaceSidebar isPersonal={true} />
        <main className="flex-1 flex flex-col">
          ...
          <div className="flex-1 p-6 bg-background overflow-auto">
            {location.pathname === "/workspace/keys" ? (
              <ApiKeys enterprise={null as any} role="member" />
            ) : location.pathname === "/workspace/stats" ? (
              <ResourceStats enterprise={null as any} role="member" />
            ) : location.pathname === "/workspace/logs" ? (
              <CallLogs enterprise={null as any} role="member" currentOrg={null} orgList={[]} />
            ) : location.pathname === "/workspace/balance" ? (
              <AccountBalance enterprise={null as any} role="member" />
              ...
              <div className="flex flex-col items-center justify-center h-[400px] text-muted-foreground">
                ...
                <h3 className="text-lg font-medium text-foreground mb-2">模型广场</h3>
                <p className="text-sm">即将上线，敬请期待</p>
              </div>
            ) : (
              <>
                <div className="mb-8">
                  <h1 className="text-2xl font-bold text-foreground">个人概览</h1>
                  <p className="text-muted-foreground mt-1">
                    欢迎回来，当前处于<span className="font-medium text-primary ml-1">个人模式</span>
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    { label: "API 密钥", value: "—", icon: Key, color: "hsl(262,60%,58%)" },
                    { label: "本月调用", value: "—", icon: TrendingUp, color: "hsl(32,90%,55%)" },
                    { label: "剩余额度", value: "—", icon: Wallet, color: "hsl(142,70%,45%)" },
                    { label: "常用模型", value: "—", icon: Link, color: "hsl(224,76%,48%)" },
                  ].map((stat) => (
                ...
                <div className="mt-6 bg-card border border-border rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {[
                      ...
                      { title: "浏览模型广场", desc: "发现适合您的 AI 模型", icon: Link },
                      { title: "查看使用文档", desc: "了解如何使用 AI 网关", icon: TrendingUp },
                    ].map((item) => (
                      <div key={item.title}
