...
function KeyTypeDonut({ data }: { data: KeyTypeData }) {
  const total = data.member + data.dept;
  const chartData = [
    { name: "成员级", value: data.member, color: "#60a5fa" },
    { name: "部门级", value: data.dept, color: "#a78bfa" },
  ];
  return (
    <div>
      <p className="text-sm font-medium text-foreground mb-4">不同类型key消耗占比</p>
      <div className="flex items-center gap-6">
        <div className="relative shrink-0">
          <ResponsiveContainer width={160} height={160}>
            <PieChart>
...
function SuccessRateDonut({ data }: { data: SuccessRateData }) {
  const total = data.success + data.fail;
  const successRate = total > 0 ? ((data.success / total) * 100).toFixed(1) : "0.0";
  const chartData = [
    { name: "成功", value: data.success, color: "#4ade80" },
    { name: "失败", value: data.fail, color: "#f87171" },
  ];
  return (
    <div>
      ...
      <div className="flex items-center gap-6">
        <div className="relative shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            ...
            <span className="text-base font-bold text-foreground">{successRate}%</span>
          </div>
        </div>
        <div className="flex-1 space-y-3">
          {chartData.map((item, i) => {
...
export default function ResourceStats({ enterprise }: Props) {
  ...
  // Data view tab: personal (我的视图) vs org/enterprise view
  const [dataViewTab, setDataViewTab] = useState<"personal" | "org">("org");
  ...
  // Effective view role for display: when org/enterprise admin selects personal view, show member-like data
  const effectiveViewRole: ViewRole =
    (viewRole === "org_admin" || viewRole === "enterprise_admin") && dataViewTab === "personal"
      ? "member"
      : viewRole;
  ...
  return (
    <div>
      ...
      {/* Data view tabs — org_admin & enterprise_admin */}
      {(viewRole === "org_admin" || viewRole === "enterprise_admin") && (
        <div className="flex items-center gap-2 mb-4">
          <div className="flex items-center bg-muted rounded-lg p-1 h-9">
            <button
              onClick={() => setDataViewTab("personal")}
              className={cn(
                "px-3 h-full rounded-md text-sm font-medium transition-all",
                dataViewTab === "personal"
      ...
      {effectiveViewRole === "enterprise_admin" && (
        <div className="bg-card border border-border rounded-xl px-4 py-3 mb-4 flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 shrink-0">
            <Building2 className="w-4 h-4 text-muted-foreground" />
      ...
      {effectiveViewRole === "org_admin" && committedMember === "" && (
        <div className="mb-4 rounded-lg bg-blue-50 border border-blue-200 px-4 py-3 flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-2 shrink-0">
            <div className="w-7 h-7 rounded-full bg-blue-100 flex items-center justify-center">
              <Wallet className="w-4 h-4 text-blue-600" />
      ...
      {effectiveViewRole === "enterprise_admin" && committedEnterpriseMember === "" && (
      ...
      {(viewRole === "member" || viewRole === "org_admin" || viewRole === "enterprise_admin") && (
        <div className="mt-4 bg-card border border-border rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {/* 不同类型key消耗占比 - 只在部门/企业视图显示 */}
            {(effectiveViewRole === "org_admin" || effectiveViewRole === "enterprise_admin") && (
              <KeyTypeDonut
                data={
                  effectiveViewRole === "enterprise_admin" && committedEnterpriseMember === ""
                    ? { member: 298.5, dept: 299.6 }
                    : effectiveViewRole === "org_admin" && committedMember === ""
                    ? { member: 88.5, dept: 100.0 }
                    : { member: 12.5, dept: 0 }
                }
              />
            )}
            {/* Top 5 Key 消耗金额排行 - 条形图 */}
            <div>
              <p className="text-sm font-medium text-foreground mb-4">Top 5 Key 消耗金额排行</p>
              <div className="space-y-3">
                {(effectiveViewRole === "enterprise_admin" && committedEnterpriseMember === ""
                  ? activeEnterpriseKeyData.slice(0, 5)
                  : effectiveViewRole === "org_admin" && committedMember === ""
                  ? mockOrgKeyConsumptionData.slice(0, 5)
                  : mockKeyConsumptionData.slice(0, 5)
                ).map((item, i) => {
                  const maxValue = (effectiveViewRole === "enterprise_admin" && committedEnterpriseMember === ""
                    ? activeEnterpriseKeyData[0]?.value || 1
                    : effectiveViewRole === "org_admin" && committedMember === ""
                    ? mockOrgKeyConsumptionData[0]?.value || 1
                    : mockKeyConsumptionData[0]?.value || 1
                  );
                  ...
                  return (
                    <div key={i} className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="text-foreground font-medium shrink-0">¥{item.value.toFixed(2)}</span>
                      </div>
            ...
            <SuccessRateDonut
              data={
                effectiveViewRole === "enterprise_admin" && committedEnterpriseMember === ""
                  ? { success: 5234, fail: 322 }
                  : effectiveViewRole === "org_admin" && committedMember === ""
                  ? { success: 1847, fail: 108 }
                  : { success: 245, fail: 30 }
              }
            />
          </div>
