...
import { AlertCircle, Check, ChevronsUpDown, Download, Eye, KeyRound, Plus, ReceiptText, RefreshCw, Trash2, WalletCards } from "lucide-react";
...
type KeyMapping = { id: number; accountId: number; key: string; credentialConfigured: boolean; customer: string; pricingMode: "uniform" | "model"; discount: string; modelDiscounts: ModelDiscount[]; enabled?: boolean };
...
const initialMappings: KeyMapping[] = [
  { id: 1, accountId: 1, key: "星海生产环境", credentialConfigured: true, customer: "星海科技有限公司", pricingMode: "uniform", discount: "85", modelDiscounts: [] },
  { id: 2, accountId: 1, key: "远山智能助手", credentialConfigured: true, customer: "远山智能有限公司", pricingMode: "uniform", discount: "90", modelDiscounts: [] },
  { id: 3, accountId: 1, key: "云杉客服应用", credentialConfigured: true, customer: "云杉网络有限公司", pricingMode: "uniform", discount: "88", modelDiscounts: [] },
  { id: 6, accountId: 2, key: "星海 GLM 生产", credentialConfigured: true, customer: "星海科技有限公司", pricingMode: "model", discount: "100", modelDiscounts: [{ model: "glm-4-plus", discount: "85" }, { model: "glm-4-air", discount: "90" }] },
  { id: 9, accountId: 3, key: "云杉百炼生产", credentialConfigured: true, customer: "云杉网络有限公司", pricingMode: "uniform", discount: "88", modelDiscounts: [] },
];
const bills: Bill[] = [
...
export default function AdminOfficialKeyData() {
  ...
  const [mappingForm, setMappingForm] = useState<{ accountId: number; key: string; secret: string; customer: string; pricingMode: "uniform" | "model"; discount: string; modelDiscounts: ModelDiscount[] }>({ accountId: initialAccounts[0].id, key: "", secret: "", customer: "", pricingMode: "uniform", discount: "100", modelDiscounts: [] });
  ...
  const openMapping = (mapping?: KeyMapping) => {
    ...
    setMappingForm(mapping ? { accountId: mapping.accountId, key: mapping.key, secret: "", customer: mapping.customer, pricingMode: mapping.pricingMode, discount: mapping.discount, modelDiscounts: mapping.modelDiscounts } : { accountId: initialAccounts[0].id, key: "", secret: "", customer: "", pricingMode: "uniform", discount: "100", modelDiscounts: [] });
    setMappingDialog(true);
  };
  const saveMapping = () => {
    const pricingValid = mappingForm.pricingMode === "uniform" ? !!mappingForm.discount : mappingForm.modelDiscounts.length > 0 && mappingForm.modelDiscounts.every((item) => item.model && item.discount);
    if (!mappingForm.accountId || !mappingForm.key || (!editingMapping?.credentialConfigured && !mappingForm.secret) || !mappingForm.customer || !pricingValid) return toast({ title: "请完善供应商、Key、密钥、客户和折扣规则", variant: "destructive" });
    const value: KeyMapping = { id: editingMapping?.id || Date.now(), accountId: mappingForm.accountId, key: mappingForm.key, credentialConfigured: editingMapping?.credentialConfigured || !!mappingForm.secret, customer: mappingForm.customer, pricingMode: mappingForm.pricingMode, discount: mappingForm.discount, modelDiscounts: mappingForm.modelDiscounts, enabled: editingMapping?.enabled ?? true };
