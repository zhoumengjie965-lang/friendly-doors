...
// Demo data for when no real orgs exist yet - hierarchical structure
const DEMO_ORGS: Org[] = [
  { id: "demo-1", name: "技术研发部",   status: "active",   admin_phone: "13800138001", monthly_budget: 20000, current_month_budget: 8400,  memberCount: 12, adminName: "陈志远", parent_id: null },
  { id: "demo-2", name: "前端工程组",   status: "active",   admin_phone: "13912340001", monthly_budget: 6000,  current_month_budget: 5800,  memberCount: 5,  adminName: "林晓雨", parent_id: "demo-1" },
  { id: "demo-3", name: "后端服务组",   status: "active",   admin_phone: "13612340002", monthly_budget: 8000,  current_month_budget: 2100,  memberCount: 6,  adminName: "王磊", parent_id: "demo-1" },
  { id: "demo-4", name: "AI算法组",     status: "active",   admin_phone: null,          monthly_budget: null,  current_month_budget: 0,     memberCount: 3,  adminName: null, parent_id: "demo-1" },
  { id: "demo-5", name: "市场运营部",   status: "active",   admin_phone: "18800110022", monthly_budget: 12000, current_month_budget: 3200,  memberCount: 9,  adminName: "张晴", parent_id: null },
  { id: "demo-6", name: "华南销售组",   status: "active",   admin_phone: "13311220033", monthly_budget: 5000,  current_month_budget: 4900,  memberCount: 7,  adminName: "刘伟强", parent_id: "demo-5" },
  { id: "demo-7", name: "品牌推广组",   status: "disabled", admin_phone: null,          monthly_budget: 4000,  current_month_budget: 0,     memberCount: 4,  adminName: null, parent_id: "demo-5" },
];
...
function buildOrgTree(orgs: Org[]): Org[] {
...
// Get all descendant org IDs (including self)
function getDescendantIds(org: Org): string[] {
  const ids = [org.id];
  if (org.children) {
...
interface OrgStructureTabProps {
  ...
  onEditMemberRole: (m: Member) => void;
  onEditMemberLimit: (m: Member) => void;
  onBanMember: (m: Member) => void;
}
...
function OrgStructureTab({
  ...
  const orgTree = buildOrgTree(orgs);

  // Get selected org and its descendants
  const selectedOrg = orgs.find((o) => o.id === selectedOrgId);
  const selectedOrgTree = selectedOrgId
    ? orgs.find((o) => o.id === selectedOrgId)
    : null;

  // Get members for selected org (including descendants if needed)
  ...
  // Expand all by default when first loaded
  useEffect(() => {
    if (orgs.length > 0 && expandedIds.size === 0) {
      const allIds = new Set<string>();
      const collectIds = (nodes: Org[]) => {
        nodes.forEach((node) => {
  ...
  // Tree node renderer
  const renderTreeNode = (org: Org, level: number = 0) => {
    ...
    return (
      <div key={org.id}>
        ...
        >
          <div className="flex items-center gap-1.5 py-2 pr-3 border-b border-border/50">
            ...
            {/* Org icon */}
            <FolderTree className={`w-4 h-4 shrink-0 ${isSelected ? "text-primary" : "text-muted-foreground"}`} />
  ...
  const currentMembers = selectedOrgId ? getOrgMembers(selectedOrgId) : [];
  ...
  return (
    <div className="bg-card border rounded-xl overflow-hidden">
      {orgs.length === 0 ? (
        ...
        <div className="grid grid-cols-[320px_1fr] min-h-[500px]">
          {/* Left: Org Tree */}
          <div className="border-r bg-muted/20">
            ...
            <div className="overflow-y-auto max-h-[600px]">
              {orgTree.map((rootOrg) => renderTreeNode(rootOrg, 0))}
            </div>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <div className="px-5 py-3 border-b bg-muted/30 flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  {selectedOrg?.name || "选择部门"}
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {currentMembers.length} 名成员
                </p>
              </div>
              {selectedOrg && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <UserCircle className="w-3.5 h-3.5" />
                  <span>
                    {selectedOrg.admin_phone
                      ? `${selectedOrg.adminName || "用户"} · ${maskPhone(selectedOrg.admin_phone)}`
                      : "未设置管理员"}
                  </span>
                </div>
              )}
            ...
            {currentMembers.length === 0 ? (
              ...
              <div className="overflow-y-auto max-h-[500px]">
                {currentMembers.map((m) => (
                  <div
                    key={m.id}
                    className="grid grid-cols-[2fr_1fr_1fr_1fr_100px] gap-3 px-5 py-3 border-b last:border-0 text-sm items-center hover:bg-muted/20"
                  >
                    <div>
