...
const DEFAULT_CUSTOMER_TIERS: CustomerTierUpgrade[] = [
  { id: genId(), customerType: "enterprise", customerName: "科技有限公司A", models: [{ modelName: "gpt-4o", rpm: 50, tpm: 100000 }, { modelName: "claude-3.5-sonnet", rpm: 80, tpm: 200000 }, { modelName: "deepseek-chat", rpm: 30, tpm: 60000 }, { modelName: "glm-4", rpm: 10, tpm: 12000 }, { modelName: "qwen-max", rpm: 20, tpm: 40000 }], remark: "年度合同客户" },
  { id: genId(), customerType: "user", customerName: "test_user_001", models: [{ modelName: "gpt-4o", rpm: 20, tpm: 50000 }], remark: "" },
  { id: genId(), customerType: "enterprise", customerName: "数据科技B", models: [{ modelName: "claude-3.5-sonnet", rpm: 100, tpm: 500000 }, { modelName: "gpt-4o", rpm: 60, tpm: 150000 }, { modelName: "gpt-4-turbo", rpm: 40, tpm: 120000 }], remark: "大促期间放宽限流" },
  { id: genId(), customerType: "user", customerName: "dev_user_042", models: [{ modelName: "gpt-4-turbo", rpm: 15, tpm: 40000 }, { modelName: "deepseek-chat", rpm: 25, tpm: 80000 }], remark: "新项目上线" },
  { id: genId(), customerType: "enterprise", customerName: "智能科技C", models: [{ modelName: "deepseek-chat", rpm: 30, tpm: 60000 }], remark: "" },
];
...
export default function RateLimitSettings() {
  ...
  // 弹窗内模型搜索 & 批量设置展开
  const [dialogModelSearchText, setDialogModelSearchText] = useState("");
  ...
  const filteredTiers = useMemo(() => {
    return customerTiers.filter((t) => {
      ...
      const matchModel = !modelSearchForTier || t.models.some((mc) => mc.modelName.toLowerCase().includes(modelSearchForTier.toLowerCase()));
      return matchCustomer && matchModel;
    });
  }, [customerTiers, selectedCustomer, modelSearchForTier]);
  ...
  const handleSaveTier = () => {
    ...
    if (editingTier.models.length === 0) {
      toast({ title: "请至少选择一个模型", variant: "destructive" });
      return;
    }
    // 校验每个模型配置
    for (const mc of editingTier.models) {
      if (mc.rpm === null && mc.tpm === null) {
        toast({ title: `模型「${mc.modelName}」的 RPM 和 TPM 至少填写一项`, variant: "destructive" });
        return;
      }
      if (mc.rpm !== null && (!Number.isInteger(mc.rpm) || mc.rpm < 1)) {
        toast({ title: `模型「${mc.modelName}」的 RPM 必须为正整数`, variant: "destructive" });
        return;
      }
      if (mc.tpm !== null && (!Number.isInteger(mc.tpm) || mc.tpm < 1)) {
        toast({ title: `模型「${mc.modelName}」的 TPM 必须为正整数`, variant: "destructive" });
        return;
      }
    }
    // 检查客户名唯一性（同一客户不能重复添加）
    ...
    if (dup) {
      toast({ title: "配置冲突", description: "该客户已存在提级配置，请编辑现有配置", variant: "destructive" });
      return;
    }
    ...
    if (editingTier.id) {
      ...
      console.log(`[审计日志] 编辑提级配置：${editingTier.customerType}=${editingTier.customerName}, 模型数=${editingTier.models.length}, 操作时间=${nowStr()}`);
    } else {
      setCustomerTiers((prev) => [...prev, { ...editingTier, id: genId() }]);
      console.log(`[审计日志] 新增提级配置：${editingTier.customerType}=${editingTier.customerName}, 模型数=${editingTier.models.length}, 操作时间=${nowStr()}`);
    }
    setCustomerDialogOpen(false);
    setEditingTier(null);
    toast({ title: "保存成功", description: "客户提级配置已保存" });
  };
  ...
  const handleConfirmDelete = () => {
    ...
    toast({
      title: "已删除",
      description: `「${t.customerName}」的提级配置已删除`,
    });
    console.log(`[审计日志] 删除提级配置：${t.customerType}=${t.customerName}, 模型数=${t.models.length}, 操作时间=${nowStr()}`);
            ...
            <table className="w-full text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y">
                {filteredModels.length === 0 ? (
                  <tr><td colSpan={7} className="px-3 py-8 text-center text-muted-foreground">无匹配数据</td></tr>
                ) : (
            ...
            <table className="w-full text-sm whitespace-nowrap">
              <thead className="bg-muted/50">
                <tr>
                  ...
                  <th className="px-3 py-2.5 text-left font-medium text-muted-foreground">提级模型</th>
              ...
              <tbody className="divide-y">
                {filteredTiers.length === 0 ? (
                  <tr><td colSpan={4} className="px-3 py-8 text-center text-muted-foreground">无匹配数据</td></tr>
                ) : (
                  filteredTiers.map((t) => {
                    return (
                        ...
                        <td className="px-3 py-2.5">
                          <div className="flex flex-wrap gap-1.5 items-center">
                            {t.models.length === 0 ? (
                              ...
                              <Popover>
                                <div className="flex flex-wrap gap-1.5 items-center">
                                  {t.models.slice(0, 2).map((mc) => (
                                    <Badge key={mc.modelName} variant="secondary" className="text-xs font-normal">{mc.modelName}</Badge>
                                  ))}
                                  {t.models.length > 2 && (
                                    <span className="text-xs text-muted-foreground">+n</span>
                                  )}
                                  <PopoverTrigger asChild>
                                    <button className="text-xs text-blue-600 hover:text-blue-700">查看</button>
                                  </PopoverTrigger>
                                </div>
                                <PopoverContent className="w-72 p-0" align="start">
                                  <div className="p-3">
                                    <div className="space-y-1">
                                      {t.models.map((mc) => (
                                        <div key={mc.modelName} className="flex items-center justify-between text-xs py-1 border-b last:border-0">
                                          <span className="font-medium">{mc.modelName}</span>
                                          <span className="text-muted-foreground tabular-nums">
                                            RPM {mc.rpm ?? "-"} / TPM {mc.tpm !== null ? mc.tpm.toLocaleString() : "-"}
                                          </span>
                                        </div>
                                      ))}
                                    </div>
        ...
        <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
          ...
          {editingTier && (
            <div className="space-y-4 py-4">
              ...
              {/* 模型选择区域 */}
              <div>
                <label className="text-sm font-medium text-gray-700 mb-2 block">提级模型 <span className="text-destructive">*</span></label>
                {!editingTier.id ? (
                  /* 新增模式：搜索选择 + 行式设置 */
                  <>
                    {/* 搜索添加模型 */}
                    <div className="relative mb-3" ref={modelSearchContainerRef}>
                      <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                      <Input
                        value={dialogModelSearchText ?? ""}
                        onChange={(e) => setDialogModelSearchText(e.target.value)}
                        placeholder="输入模型名称搜索..."
                        className="pl-8 h-9"
                      />
                      {dialogModelSearchText && (() => {
                        const q = dialogModelSearchText.toLowerCase();
                        const matched = modelNames.filter((mn) => mn.toLowerCase().includes(q));
                        if (matched.length === 0) return null;
                        return (
                          <div className="absolute z-20 w-full mt-1 bg-popover border rounded-lg shadow-lg max-h-[200px] overflow-y-auto">
                            {matched.map((mn) => {
                              const isAdded = editingTier.models.some((mc) => mc.modelName === mn);
                              return (
                                <button
                                  key={mn}
                                  className={`w-full px-3 py-2 text-left hover:bg-accent transition-colors text-sm border-b last:border-b-0 flex items-center justify-between ${
                                    isAdded ? "bg-green-50 text-green-700" : ""
                                  }`}
                                  onClick={() => {
                                    if (isAdded) {
                                      setEditingTier({ ...editingTier, models: editingTier.models.filter((mc) => mc.modelName !== mn) });
                                    } else {
                                      setEditingTier({ ...editingTier, models: [...editingTier.models, { modelName: mn, rpm: null, tpm: null }] });
                                    }
                                  }}
                                >
                                  <span>{mn}</span>
                                  {isAdded && (
                                    <span className="text-green-600 text-xs font-medium">已添加</span>
                                  )}
                                </button>
                              );
                            })}
                    ...
                    {/* 已选模型列表 — 一行一个：模型名 | RPM | TPM | 删除 */}
                    {editingTier.models.length > 0 ? (
                      <div className="space-y-1.5">
                        {/* 批量设置：按钮+输入框同一行 */}
                        {editingTier.models.length > 1 && (
                          <div className="flex items-center gap-2 bg-muted/30 rounded-md px-2 py-1.5">
                            <button
                              type="button"
                              className="text-xs text-blue-600 hover:text-blue-700 flex items-center gap-1"
                              onClick={() => setShowBatchSet(!showBatchSet)}
                            >
                              <span>批量填写</span>
                              <ChevronRight className={`w-3 h-3 transition-transform ${showBatchSet ? "rotate-90" : ""}`} />
                            </button>
                            {showBatchSet && (
                              <>
                                <span className="text-xs text-muted-foreground shrink-0 ml-2">RPM:</span>
                                <Input
                                  type="number"
                                  min={1}
                                  placeholder="-"
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setEditingTier({ ...editingTier, models: editingTier.models.map((mc) => ({ ...mc, rpm: val })) });
                                  }}
                                />
                                <span className="text-xs text-muted-foreground shrink-0">TPM:</span>
                                <Input
                                  type="number"
                                  min={1}
                                  placeholder="-"
                                  ...
                                  onChange={(e) => {
                                    const val = e.target.value === "" ? null : Number(e.target.value);
                                    setEditingTier({ ...editingTier, models: editingTier.models.map((mc) => ({ ...mc, tpm: val })) });
                                  }}
                                />
                              </>
                            )}
                        ...
                        {/* 逐行设置 */}
                        {editingTier.models.map((mc, idx) => (
                          <div key={mc.modelName} className="flex items-center gap-2 bg-muted/30 rounded-md px-2 py-1.5">
                            <span className="text-sm font-medium text-foreground shrink-0 min-w-[140px]">{mc.modelName}</span>
                            <span className="text-xs text-muted-foreground shrink-0">RPM:</span>
                            <Input
                              type="number"
                              min={1}
                              value={mc.rpm ?? ""}
                              onChange={(e) => {
                                const val = e.target.value === "" ? null : Number(e.target.value);
                                const newModels = [...editingTier.models];
                                newModels[idx] = { ...newModels[idx], rpm: val };
                                setEditingTier({ ...editingTier, models: newModels });
                              }}
                              placeholder="-"
                              className="h-7 w-20 text-xs"
                            />
                            ...
                            <Input
                              type="number"
                              min={1}
                              value={mc.tpm !== null ? mc.tpm.toLocaleString() : ""}
                              onChange={(e) => {
                                const val = e.target.value === "" ? null : Number(e.target.value);
                                const newModels = [...editingTier.models];
                                newModels[idx] = { ...newModels[idx], tpm: val };
                                setEditingTier({ ...editingTier, models: newModels });
                              }}
                              placeholder="-"
                              className="h-7 w-24 text-xs"
                            ...
                            <button
                              ...
                              onClick={() =>
                                setEditingTier({ ...editingTier, models: editingTier.models.filter((m) => m.modelName !== mc.modelName) })
                              }
                              title="移除此模型"
                            >
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {isAdded ? (
                                    ...
                                    <span className="text-blue-600 text-xs font-medium">点击添加</span>
                                  )}
                                </button>
                              );
                            })}
