...
import {
  ...
  ChevronDown, ChevronRight, Wallet, Settings, UserCircle
} from "lucide-react";
...
export default function WorkspaceSidebar({ enterpriseName, enterpriseCode, isPersonal = false }: Props) {
  const location = useLocation();
  const navItems = isPersonal ? personalNavItems : enterpriseNavItems;
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-white text-sm truncate">
              {isPersonal ? "个人空间" : enterpriseName}
            </p>
            <p className="text-xs text-white/50 font-mono">
              {isPersonal ? "个人模式" : enterpriseCode}
            </p>
          </div>
